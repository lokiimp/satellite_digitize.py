...
def find_32A(data):
    for ln in data["readResult"]["blocks"][0]["lines"]:
        if any(sub in ln["text"] for sub in ("32A-2",)):
...
save_folder = os.path.join(base,"shift_off_satname")
...
for doy in range(start_day, max_day + 1):
    ...
    for file in os.listdir(full_path):
        if file.lower().endswith(".vi.thumb.png"):
            ...
            coords = find_32A(data)
            if coords is None:
                print(f"No 32A-2 block in {json_file}")
                continue

            xshift = 51 - int(coords["x"])
            yshift = 14 - int(coords["y"])
            ...
            out_path = os.path.join(save_folder, file_base + ".vi.thumb.shift.png")

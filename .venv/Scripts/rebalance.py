...
def grayworld_whitebalance(img_bgr: np.ndarray) -> np.ndarray:
    # convert to float32 so scaling isn’t clipped prematurely
    ...
    # apply scaling and clip back to [0,255]
...
out = cv.VideoWriter(os.path.join(base,f"jsonshifted_cleanish_brush{brush_size}_fullautowb({wb})_nobg.mp4"), fourcc,10,(2000,2000))
save_folder = os.path.join(base,f"jsonshifted_cleanish_brush{brush_size}_fullautowb({wb})_nobg")
...
for doy in range(start_day, (366 if year%4==0 else 365)+1):
    ...
    for fname in os.listdir(full):
        ...
        out_path = os.path.join(save_folder, fb + f".vi.med.jsonshift.brush{brush_size}.low{lw}.fullautowb.png")

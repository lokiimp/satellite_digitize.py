...
OUTPUT_DIR = INPUT_DIR.replace("aligned_with_grid", "aligned_with_grid_color_and_whitebalanced_2")
DEBUG_BAR_DIR = INPUT_DIR.replace("aligned_with_grid", "wb_debug_bar")
BASE_PATH = INPUT_DIR.replace("/1977/grid_aligned/aligned_output_vi_2/aligned_with_grid", "")
SAT = INPUT_DIR.replace("/arc25/arcdata/alpha/goes/pregvar/", "").replace("/vissr/1977/grid_aligned/aligned_output_vi_2/aligned_with_grid","")
...
def main():
    with open(SHIFT_LOG, 'w') as log:
        for img_path in sorted(glob.glob(os.path.join(INPUT_DIR, '*.png'))):
            ...
            # Parse date/time from grid_aligned filename
            # Assuming format: goes02.YYYY.DDD.HHMMSS.png
            m = re.match(f"{SAT}"+r"\.(\d{4})\.(\d{3})\.(\d{6})\.png", fname)
            if not m:
                log.write(f"{fname}: filename parse failed\n")
                continue
            yyyy, ddd, hhmmss = m.groups()

            # Convert YYYY + DDD to MM/DD for med image path
            from datetime import datetime
            dt = datetime.strptime(f"{yyyy} {ddd}", "%Y %j")
            mm = dt.strftime("%m")
            dd = dt.strftime("%d")

            # Build path to matching med image
            med_path = f"{BASE_PATH}/{yyyy}/{yyyy}_{mm}_{dd}_{ddd}/{SAT}.{yyyy}.{ddd}.{hhmmss}.vi.med.png"
            med_img = cv2.imread(med_path)
            if med_img is None:
                log.write(f"{fname}: med image not found at {med_path}\n")
                continue

            # Use med image to find bar shift
            gray_med = cv2.cvtColor(med_img, cv2.COLOR_BGR2GRAY)
            img_bin = threshold_bin(gray_med, BRIGHT_THRESH)
            tf = find_bar_transform(img_bin, bar_mask)

            # Align mask to med image
            h, w = bar_mask.shape
            ...
            if len(ys) == 0:
                log.write(f"{fname}: bar not found in med image\n")
                continue
            ...
            vals = gray_med[y_sample, sample_x].astype(float)
            tgt = np.linspace(0, 255, N_SAMPLES)
            diffs = tgt - vals
            diffs[np.abs(diffs) > 65] = 0
            avg_shift = diffs.mean()

            # White balance reference from med image
            wb_x = sample_x[-3]
            h_img, w_img = med_img.shape[:2]
            wb_y = min(y_sample + 50, h_img - 1)
            bx, gx, rx = med_img[wb_y, wb_x]
            gb = 255.0 / bx if bx > 0 else 1.0
            gg = 255.0 / gx if gx > 0 else 1.0
            gr = 255.0 / rx if rx > 0 else 1.0

            # Now apply to the *grid_aligned* image
            aligned_img = cv2.imread(img_path)
            if aligned_img is None:
                log.write(f"{fname}: grid_aligned load failed\n")
                continue

            out = np.clip(aligned_img.astype(np.float32) + avg_shift, 0, 255).astype(np.uint8)
            ...
            # Debug bar overlay from med image
            dbg_bar = med_img.copy()
            ...
            log.write(f"{fname} vals: " + ", ".join(f"{v:.2f}" for v in vals) + "\n")
            log.write(f"{fname} diffs: " + ", ".join(f"{d:.2f}" for d in diffs) + "\n")
            log.write(f"{fname}: angle={tf['angle']:.2f}, dx={tf['dx']:.1f}, dy={tf['dy']:.1f}, shift={avg_shift:.2f}, wb_gains=({gb:.2f},{gg:.2f},{gr:.2f})\n")
            print(f"{fname}: applied shift {avg_shift:.2f}, white-balance gains=({gb:.2f},{gg:.2f},{gr:.2f})")

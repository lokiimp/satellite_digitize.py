...
OUTPUT_DIR     = "/ships22/sds/goes/digitized/32A/vissr/1978/grid_aligned/aligned_output_vi_4/aligned_with_grid_colorbalanced_6"
DEBUG_BAR_DIR  = "/ships22/sds/goes/digitized/32A/vissr/1978/grid_aligned/aligned_output_vi_4/aligned_with_grid_debug_bar_6"
...
for d in (OUTPUT_DIR, DEBUG_BAR_DIR, DEBUG_MASK_DIR):
...
HORIZ_START_OFS= 70    # pixels to shift start right
HORIZ_END_OFS  = 40     # pixels to shift end left
...
def main():
    with open(SHIFT_LOG, 'w') as log:
        for img_path in sorted(glob.glob(os.path.join(INPUT_DIR, '*.png'))):
            ...
            log.write(f"{fname}: angle={tf['angle']:.2f}, dx={tf['dx']:.1f}, dy={tf['dy']:.1f}, shift={avg_shift:.2f}\n")
            print(f"{fname}: applied shift {avg_shift:.2f}")

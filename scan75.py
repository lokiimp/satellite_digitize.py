...
def button_press():
    ...
    # --- clear out the time field for the next scan ---
    time_entry.delete(0, tk.END)

    # now handle toggling satellite suffix exactly like before
    sat_input = vals[0]
    if sat_input.endswith('-A-2'):
        prefix, suffix = sat_input[:-4], '-A-2'
    elif sat_input.endswith('-A'):
        prefix, suffix = sat_input[:-2], '-A'
    else:
        prefix, suffix = sat_input, ''
    ...
    satellite_entry.insert(0, prefix + new_suffix)
